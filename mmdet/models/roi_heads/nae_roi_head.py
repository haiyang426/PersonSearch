...
class NaeRoIHead(BaseRoIHead):
    
    def __init__(self, with_fcs: bool, **kwargs):
    ...
    def extract_with_bbox(self,
        ...
        proposals = [res.bboxes * res.bboxes.new_tensor(s['scale_factor']).repeat((1, 2)) 
                        for res, s in zip(rpn_results_list, batch_img_metas)]

        num_proposals_per_img = tuple(len(p) for p in proposals)
        rois = bbox2roi(proposals)

        bbox_results = self._bbox_forward(x, rois)
        id_preds = bbox_results['id_pred']
        id_preds = id_preds.split(num_proposals_per_img, 0)
        return id_preds
    ...
    def predict_bbox(self,
        ...
        id_preds = id_preds.split(num_proposals_per_img, 0)
        if self.with_fcs:
            cls_scores = [r.scores.unsqueeze(1) for r in rpn_results_list]
        else:
            cls_scores = torch.sigmoid(bbox_results['cls_score'])
            cls_scores = cls_scores.split(num_proposals_per_img, 0)
