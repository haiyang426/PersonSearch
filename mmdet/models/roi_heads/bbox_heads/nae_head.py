...
from mmdet.models.layers import multiclass_nms, multiclass_embedding_nms
...
class NaeHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_bbox: ConfigType = dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=10.0),
                 loss_id: ConfigType=dict(
                     type='OIMLoss', 
                     num_features=256,
                     num_pids=482,
                     num_cq_size=500,
        ...
        if self.with_id:
            ...
            self.fc_reid_before = nn.Sequential(
                    nn.Linear(in_channels//2, 128), 
                    nn.BatchNorm1d(128))
            nn.init.normal_(self.fc_reid_before[0].weight, std=0.01)
            nn.init.normal_(self.fc_reid_before[1].weight, std=0.01)
            nn.init.constant_(self.fc_reid_before[0].bias, 0)
            nn.init.constant_(self.fc_reid_before[1].bias, 0)
            
            self.fc_reid_after = nn.Sequential(
                    nn.Linear(in_channels, 128), 
                    nn.BatchNorm1d(128))
            nn.init.normal_(self.fc_reid_after[0].weight, std=0.01)
            nn.init.normal_(self.fc_reid_after[1].weight, std=0.01)
            nn.init.constant_(self.fc_reid_after[0].bias, 0)
            nn.init.constant_(self.fc_reid_after[1].bias, 0)
            
            self.fc_reg_bn = nn.BatchNorm1d(4 * num_classes)
            nn.init.normal_(self.fc_reg_bn.weight, std=0.01)
            nn.init.constant_(self.fc_reg_bn.bias, 0)
        # if with_share_feat:
    ...
    def _extract(self, feat_p_before, feat_p_after):
        
        # feat_p_before = F.adaptive_max_pool2d(feat_p_before, 1).flatten(1)
        feat_p_after = F.adaptive_max_pool2d(feat_p_after, 1).flatten(1)
        # id_pred_before = F.normalize(self.fc_reid1(feat_p_before))
    ...
    def forward(self, feat_p_before: Tuple[Tensor]) -> tuple:
        ...
        # print(feat_p_after.shape)
        # print(feat_p_before.shape)
        # exit()
        # eat_p_after = F.adaptive_max_pool2d(feat_p_after, 1).flatten(1)
        # feat_p_after = x
        cls_score = self.fc_cls(feat_p_after) if self.with_cls else None
        bbox_pred = self.fc_reg(feat_p_after) if self.with_reg else None

        if self.with_id:
            id_pred_after = self.fc_reid_after(feat_p_after)
            id_pred_before = self.fc_reid_before(feat_p_before)
            
            id_pred = torch.cat([id_pred_before, id_pred_after], dim=1)
            
            norms = id_pred.norm(2, 1, keepdim=True)
            id_pred = id_pred/norms.expand_as(id_pred).clamp(min=1e-12)
            cls_score = self.rescaler(norms)
            bbox_pred = self.fc_reg_bn(bbox_pred)
    ...
    def _get_targets_single(self, pos_priors: Tensor, neg_priors: Tensor,
                            ...
                            pos_gt_ids: Tensor, cfg: ConfigDict) -> tuple:
        ...
        return labels, label_weights, bbox_targets, bbox_weights, ids, ids_weights
    ...
    def get_targets(self,
        ...
        pos_gt_ids_list = [res.pos_gt_ids for res in sampling_results]
        labels, label_weights, bbox_targets, bbox_weights, ids, ids_weights = multi_apply(
        ...
        if concat:
            ...
            ids = torch.cat(ids, 0)
            ids_weights = torch.cat(ids_weights, 0)
        return labels, label_weights, bbox_targets, bbox_weights, ids, ids_weights
    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if isinstance(loss_cls_, dict):
                    ...
                    losses['loss_reid_cls'] = loss_cls_
                if self.custom_activation:
                    acc_ = self.loss_cls.get_accuracy(cls_score, labels)
                    losses.update(acc_)
                else:
                    losses['reid_acc'] = accuracy(cls_score, labels)
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_reid_bbox'] = self.loss_bbox(
                    pos_bbox_pred,
                ...
                losses['loss_reid_bbox'] = bbox_pred[pos_inds].sum()
                
        # ids_weights  
        if id_pred is not None:
            losses['loss_reid_id'] = self.loss_id(id_pred[ids_weights>0], ids[ids_weights>0])
        return losses
    ...
    def _predict_by_feat_single(
        ...
        if rcnn_test_cfg is None:
            ...
            if id_pred is not None:
                det_bboxes, det_labels, det_ids = multiclass_embedding_nms(
                    bboxes,
                    scores,
                    id_pred,
                ...
                results.ids = det_ids * results.scores.view(-1, 1)
            else:
